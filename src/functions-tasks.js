...
function logger(func, logFunc) {
  return (...args) => {
    const stringArguments = args.map((arg) => JSON.stringify(arg));
    logFunc(`${func.name}(${stringArguments.join(',')}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${stringArguments.join(',')}) ends`);
    return result;
  };
}
